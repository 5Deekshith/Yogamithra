...
import 'package:yoga_mithra/Authtentication/signup.dart';
import 'package:yoga_mithra/JsonModels/users.dart';
import 'package:yoga_mithra/SQLite/sqlite.dart';
import 'package:yoga_mithra/Views/home_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  login() async {
    int mobile = 0;
    try {
      mobile = int.parse(username.text);
    } catch (e) {}
    Users? response = await db.login(Users(
        usrName: username.text,
        usrPassword: password.text,
        email: username.text,
        mobileNumber: mobile));
    if (response != null) {
      //If login is correct, then goto yoga_mithra
      if (!mounted) return;
      Navigator.pushReplacement(
          context, MaterialPageRoute(builder: (context) => HomeScreen(user: response,)));
    } else {
